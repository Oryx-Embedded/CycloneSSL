   ...
   TlsEncryptionLevel level, const uint8_t *secret)
{
      ...
      context->version == TLS_VERSION_1_3)
   {
      ...
      if(!error)
      {
            ...
            encryptionEngine->cipherMode == CIPHER_MODE_CHACHA20_POLY1305)
         {
            ...
            if(hashAlgo != NULL)
            {
               //Calculate the sequence number encryption key (refer to
               //RFC 9147, section 4.2.3)
               error = tls13HkdfExpandLabel(context->transportProtocol,
                  hashAlgo, secret, hashAlgo->digestSize, "sn", NULL, 0,
   ...
   {
      //Default lifetime
      encryptionEngine->lifetime = 0;
      encryptionEngine->timestamp = osGetSystemTime();

      //Operational state of the encryption engine
      encryptionEngine->active = TRUE;
   }
...
/**
 * @brief Update encryption engine
 * @param[in] context Pointer to the TLS context
 * @param[in] encryptionEngine Pointer to the encryption/decryption engine to
 *   be initialized
 * @param[in] entity Specifies whether client or server write keys shall be used
...
error_t tlsUpdateEncryptionEngine(TlsContext *context,
   TlsEncryptionEngine *encryptionEngine, TlsConnectionEnd entity,
   TlsEncryptionLevel level, const uint8_t *secret)
{
   error_t error;
   ...
   if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
   {
      ...
      if(encryptionEngine == context->encryptionEngine)
      {
#if (TLS_MAX_VERSION >= TLS_VERSION_1_3 && TLS_MIN_VERSION <= TLS_VERSION_1_3)
         //DTLS 1.3 protocol?
         if(context->version == TLS_VERSION_1_3)
         {
            //Release oldest encryption engine first
            tlsFreeEncryptionEngine(&context->encryptionEngine[2]);

            //Save encryption engines for later use
            context->encryptionEngine[2] = context->encryptionEngine[1];
            context->encryptionEngine[1] = *encryptionEngine;
         }
         else
#endif
         {
            //Release previous encryption engine first
            tlsFreeEncryptionEngine(&context->encryptionEngine[1]);

            //Save current encryption engine for later use
            context->encryptionEngine[1] = *encryptionEngine;
         }

         //Reset current encryption engine
         osMemset(encryptionEngine, 0, sizeof(TlsEncryptionEngine));
         encryptionEngine->epoch = context->encryptionEngine[1].epoch;
      }
      else if(encryptionEngine == context->decryptionEngine)
      {
#if (TLS_MAX_VERSION >= TLS_VERSION_1_3 && TLS_MIN_VERSION <= TLS_VERSION_1_3)
         //DTLS 1.3 protocol?
         if(context->version == TLS_VERSION_1_3)
         {
            //Release oldest encryption engine first
            tlsFreeEncryptionEngine(&context->decryptionEngine[1]);

            //Save encryption engines for later use
            context->decryptionEngine[1] = *encryptionEngine;

            //Reset current encryption engine
            osMemset(encryptionEngine, 0, sizeof(TlsEncryptionEngine));
            encryptionEngine->epoch = context->decryptionEngine[1].epoch;
         }
         else
#endif
         {
         ...
         dtlsInitReplayWindow(encryptionEngine);
      }
      else
      {
         //Report an error
   ...
   if(!error)
   {
      //Re-initialize encryption engine
      error = tlsInitEncryptionEngine(context, encryptionEngine, entity, level,
         secret);
   }
