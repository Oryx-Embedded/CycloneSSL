...
#include "tls/tls_client_extensions.h"
#include "tls/tls_client_misc.h"
...
#include "tls13/tls13_client.h"
#include "tls13/tls13_client_extensions.h"
   ...
   if(!error)
   {
      ...
      if(context->versionMax >= TLS_VERSION_1_3)
      {
         ...
         else
         {
            ...
            tlsFreeEncryptionEngine(&context->encryptionEngine[0]);
         }
      }
   ...
   *length += n;
   ...
   if(context->versionMax >= TLS_VERSION_1_3)
   {
         ...
         context->version == TLS_VERSION_1_3)
      {
         ...
         error = tls13FormatClientCookieExtension(context, p, &n);
         //Any error to report?
         if(error)
            return error;
   ...
   const TlsServerHello *message, size_t length)
{
   ...
   if(context->version == TLS_VERSION_1_3)
   {
      ...
      if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
      {
         //DTLS 1.3 clients must abort the handshake with an illegal_parameter
         //alert if the legacy_session_id_echo field is not empty
      ...
      else
      {
         ...
         //match what it sent in the ClientHello must abort the handshake with
         //an illegal_parameter alert (RFC 8446, section 4.1.3)
         if(message->sessionIdLen != context->sessionIdLen ||
            osMemcmp(message->sessionId, context->sessionId,
