...
TlsContext *tlsInit(void)
{
   ...
   if(context != NULL)
   {
      ...
      context->encryptionEngine[0].version = TLS_MIN_VERSION;
      ...
      context->encryptionEngine[0].level = TLS_ENCRYPTION_LEVEL_INITIAL;
      context->decryptionEngine[0].level = TLS_ENCRYPTION_LEVEL_INITIAL;
#endif
   ...
   uint16_t versionMax)
{
   ...
   context->encryptionEngine[0].version = context->versionMin;
...
error_t tlsConnect(TlsContext *context)
{
      ...
      context->state == TLS_STATE_EARLY_DATA)
   {
      ...
      context->earlyDataSeqNum = context->encryptionEngine[0].seqNum;
      ...
      if(context->version <= TLS_VERSION_1_2)
      {
         ...
         if(context->resume)
         {
            ...
            *length = context->clientVerifyDataLen;
            ...
            //then the client must disable any use of the "tls-unique" channel
            //binding on the current connection (refer to RFC 7627, section 5.4)
            error = (context->emsExtReceived) ? NO_ERROR : ERROR_FAILURE;
#else
            //The session does not use the extended master secret
            error = ERROR_FAILURE;
#endif
         }
         else
   ...
   while(*received < size)
   {
         ...
         context->state == TLS_STATE_KEY_UPDATE_ACK)
      {
         ...
         if(!error)
         {
            ...
            else if(contentType == TLS_TYPE_ACK)
            {
               //Advance data pointer
               context->rxBufferPos += n;
               //Number of bytes still pending in the receive buffer
               ...
               //Parse ACK message
               error = dtls13ParseAck(context, (Dtls13Ack *) p, n);
            }
#endif
...
error_t tlsTick(TlsContext *context)
{
   ...
   //Valid DTLS context?
   if(context != NULL &&
      context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
   {
...
void tlsFree(TlsContext *context)
{
   ...
   if(context != NULL)
   {
      ...
      //Release encryption engines
      for(i = 0; i < TLS_MAX_ENCRYPTION_ENGINES; i++)
      {
         tlsFreeEncryptionEngine(&context->encryptionEngine[i]);
      }

      //Release decryption engines
