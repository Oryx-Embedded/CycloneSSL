...
#include "tls/tls_legacy.h"
#include "tls13/tls13_misc.h"
...
//Number of encryption engines
#if (DTLS_SUPPORT == ENABLED && TLS_MAX_VERSION >= TLS_VERSION_1_3)
   #define TLS_MAX_ENCRYPTION_ENGINES 3
#elif (DTLS_SUPPORT == ENABLED && TLS_MAX_VERSION <= TLS_VERSION_1_2)
   #define TLS_MAX_ENCRYPTION_ENGINES 2
#else
   #define TLS_MAX_ENCRYPTION_ENGINES 1
#endif

//Number of decryption engines
#if (DTLS_SUPPORT == ENABLED && TLS_MAX_VERSION >= TLS_VERSION_1_3)
   #define TLS_MAX_DECRYPTION_ENGINES 2
#else
   #define TLS_MAX_DECRYPTION_ENGINES 1
#endif
...
typedef enum
{
   ...
   TLS_STATE_NEW_SESSION_TICKET          = 23,
   TLS_STATE_NEW_SESSION_TICKET_2        = 24,
   TLS_STATE_SERVER_CHANGE_CIPHER_SPEC   = 25,
   TLS_STATE_SERVER_CHANGE_CIPHER_SPEC_2 = 26,
   TLS_STATE_SERVER_FINISHED             = 27,
   TLS_STATE_END_OF_EARLY_DATA           = 28,
   TLS_STATE_SERVER_APP_TRAFFIC_KEYS     = 29,
   TLS_STATE_FINAL_ACK                   = 30,
   TLS_STATE_APPLICATION_DATA            = 31,
   TLS_STATE_CLIENT_FINISHED_ACK         = 32,
   TLS_STATE_NEW_SESSION_TICKET_ACK      = 33,
   TLS_STATE_KEY_UPDATE                  = 34,
   TLS_STATE_KEY_UPDATE_ACK              = 35,
   TLS_STATE_CLOSING                     = 36,
   TLS_STATE_CLOSED                      = 37
} TlsState;
...
struct _TlsEncryptionEngine
{
   bool_t active;                 ///<Operational state of the encryption engine
   systime_t timestamp;           ///<Timestamp to manage lifetime
   systime_t lifetime;            ///<Lifetime of the encryption engine
   ...
   uint32_t replayWindow[(DTLS_REPLAY_WINDOW_SIZE + 31) / 32]; ///<Replay window
#endif
#if (DTLS_SUPPORT == ENABLED && TLS_MAX_VERSION >= TLS_VERSION_1_3)
   uint8_t snKey[32];             ///<Sequence number encryption key
   void *snCipherContext;         ///<Sequence number encryption context
   Dtls13RetransmitState retransmitState; ///<Retransmission state
#endif
#if (TLS_QUIC_SUPPORT == ENABLED)
...
struct _TlsContext
{
   ...
   bool_t wrongCookie;                       ///<Invalid cookie
   ...
   bool_t wrongKeyShare;                     ///<Invalid key share
   ...
   TlsEncryptionEngine encryptionEngine[TLS_MAX_ENCRYPTION_ENGINES];  ///<Encryption engines
   TlsEncryptionEngine decryptionEngine[TLS_MAX_DECRYPTION_ENGINES];  ///<Decryption engines
...
#if (DTLS_SUPPORT == ENABLED && TLS_MAX_VERSION >= TLS_VERSION_1_3)
   uint8_t clientHelloDigest[48];            ///<Hash(ClientHello1)
   size_t clientHelloDigestLen;              ///<Length of Hash(ClientHello1)
   Dtls13RecordNumber rxRecordNum;           ///<Epoch/sequence number pair
   Dtls13RecordNumber ackRecords[DTLS13_MAX_ACK_RECORDS]; ///<List of records received and processed
   uint_t numAckRecords;                     ///<Number of records in the list
   bool_t ackTimerRunning;                   ///<The ACK timer is running
   systime_t ackTimestamp;                   ///<Time at which the ACK timer started
#endif
