   ...
   const Tls13PskIdentity *identity, uint8_t *binder, size_t binderLen)
{
   ...
   //TLS 1.3-style handshake messages without the message_seq, fragment_offset,
   //and fragment_length values (refer to RFC 9147, section 5.2)
...
error_t tls13DigestClientHello1(TlsContext *context)
{
   ...
   {
      //Restore Hash(ClientHello1)
      osMemcpy(message->data, context->clientHelloDigest, hash->digestSize);
   }
   else
   {
      ...
      //Save Hash(ClientHello1)
      osMemcpy(context->clientHelloDigest, message->data, hash->digestSize);
      context->clientHelloDigestLen = hash->digestSize;
   }
#else
   //Compute Hash(ClientHello1)
   hash->final(context->transcriptHashContext, message->data);
