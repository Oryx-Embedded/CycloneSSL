...
#include "tls/tls_server_extensions.h"
#include "tls/tls_server_misc.h"
#include "tls/tls_extensions.h"
#include "tls/tls_transcript_hash.h"
#include "tls/tls_ffdhe.h"
#include "tls/tls_misc.h"
#include "tls13/tls13_server.h"
#include "tls13/tls13_server_extensions.h"
...
error_t tls13SendHelloRetryRequest(TlsContext *context)
{
   ...
   if(!error)
   {
      ...
      {
         //Generate a cookie
         error = dtls13GenerateCookie(context);
      }
   }
#endif
...
error_t tls13SendNewSessionTicket(TlsContext *context)
{
   ...
   else
   {
      ...
      if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
      {
         //When a handshake flight is sent without any expected response, as is
         //the case with the NewSessionTicket message, the flight must be
         //acknowledged with an ACK message (refer to RFC 9147, section 5.7)
         tlsChangeState(context, TLS_STATE_NEW_SESSION_TICKET_ACK);
      }
      else
      {
...
#if (DTLS_SUPPORT == ENABLED)
   //When sending a HelloRetryRequest, the server may provide a Cookie extension
   //to the client (refer to RFC 8446, section 4.2.2)
   if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM &&
      context->wrongCookie)
   {
      //The cookie provides a measure of DoS protection and allows the server to
      //offload state to the client
      error = dtls13FormatServerCookieExtension(context, p, &n);
      //Any error to report?
      if(error)
         return error;
