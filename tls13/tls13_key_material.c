...
error_t tls13GenerateEarlyTrafficKeys(TlsContext *context)
{
   ...
   else
   {
      ...
      {
         //Calculate client early traffic keys
         error = tlsUpdateEncryptionEngine(context, context->decryptionEngine,
            TLS_CONNECTION_END_CLIENT, TLS_ENCRYPTION_LEVEL_EARLY_DATA,
            context->clientEarlyTrafficSecret);
      }
      else
...
error_t tls13GenerateClientAppTrafficKeys(TlsContext *context)
{
   ...
   if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
   {
      ...
      //the case with the client's final flight, the flight must be
      //acknowledged with an ACK message (refer to RFC 9147, section 5.7)
      if(context->entity == TLS_CONNECTION_END_CLIENT)
      {
         tlsChangeState(context, TLS_STATE_CLIENT_FINISHED_ACK);
      }
      else
      {
         tlsChangeState(context, TLS_STATE_FINAL_ACK);
      }
   }
   else
   {
#if (TLS_TICKET_SUPPORT == ENABLED)
      //Check whether session ticket mechanism is enabled at server side
