   ...
   TlsHelloExtensions *extensions)
{
   ...
   if(context->transportProtocol == TLS_TRANSPORT_PROTOCOL_DATAGRAM)
   {
      ...
      //the cookie is valid (refer to RFC 9147, section 5.1)
      error = dtls13ParseClientCookieExtension(context, extensions->cookie);
      //Any error to report?
      if(error)
         return error;
   }
...
/**
 * @brief Hash HelloRetryRequest in the transcript
 * @param[in] context Pointer to the TLS context
 * @param[in] namedGroup Named group
 * @return Error code
 **/

error_t tls13DigestHelloRetryRequest(TlsContext *context,
   TlsNamedGroup namedGroup)
{
   ...
   //Use the same extensions as the original HelloRetryRequest message
   context->wrongKeyShare = (namedGroup != TLS_GROUP_NONE) ? TRUE : FALSE;
   context->wrongCookie = TRUE;

   //Format HelloRetryRequest message
   error = tls13FormatHelloRetryRequest(context,
      (Tls13HelloRetryRequest *) message->data, &length);
   //Any error to report?
   if(error)
      return error;
   ...
   //Update the hash value with the contents of the message
   hash->update(context->transcriptHashContext, message,
      length + sizeof(TlsHandshake));

   //Clear flags
   context->wrongKeyShare = FALSE;
   context->wrongCookie = FALSE;
