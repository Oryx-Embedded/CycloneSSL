...
error_t dtlsSelectVersion(TlsContext *context, uint16_t version)
{
   ...
   {
      //Initial handshake?
      if(context->encryptionEngine[0].epoch == 0)
      {
         //Save the negotiated version
         context->encryptionEngine[0].version = context->version;
      }
   }
...
error_t dtlsFormatCookie(TlsContext *context, uint8_t *p, size_t *written)
{
   ...
   //Version of DTLS prior to TLS 1.3?
   if(context->version <= TLS_VERSION_1_2)
   {
      ...
      if(context->state == TLS_STATE_CLIENT_HELLO_2)
      {
         ...
         //(refer to RFC 6347 section 4.2.1)
...
void dtlsInitReplayWindow(TlsEncryptionEngine *decryptionEngine)
{
   ...
   {
      decryptionEngine->replayWindow[i] = 0;
   }
#endif
}
...
error_t dtlsCheckReplayWindow(TlsEncryptionEngine *decryptionEngine,
   const DtlsSequenceNumber *seqNum)
{
   ...
   right = LOAD48BE(&decryptionEngine->dtlsSeqNum);
   ...
   if(n <= right)
   {
      ...
      if(n < DTLS_REPLAY_WINDOW_SIZE)
      {
         ...
         //window
         if(decryptionEngine->replayWindow[j] & (1U << k))
         {
         ...
         else
         {
            ...
            //the receiver proceeds to MAC verification
            error = NO_ERROR;
         }
...
void dtlsUpdateReplayWindow(TlsEncryptionEngine *decryptionEngine,
   const DtlsSequenceNumber *seqNum)
{
   uint64_t n;
   uint64_t right;
   ...
   if(n <= right)
   {
      ...
      if(n < DTLS_REPLAY_WINDOW_SIZE)
      {
         ...
         decryptionEngine->replayWindow[j] |= 1U << k;
      }
#endif
   }
   else
   {
      ...
      if(n < DTLS_REPLAY_WINDOW_SIZE)
      {
         ...
         if(j > 0)
         {
            ...
            {
               decryptionEngine->replayWindow[i] =
                  decryptionEngine->replayWindow[i - j];
            }
         ...
         if(k > 0)
         {
            ...
            {
               decryptionEngine->replayWindow[i] =
                  (decryptionEngine->replayWindow[i] << k) |
                  (decryptionEngine->replayWindow[i - 1] >> (32 - k));
            }

            //Shift the least significant word
            decryptionEngine->replayWindow[0] <<= k;
         }
      }
      else
      {
      ...
      //Set the corresponding bit in the bitmap window
      decryptionEngine->replayWindow[0] |= 1;
#endif

      //Save the highest sequence number value received on this session
      decryptionEngine->dtlsSeqNum = *seqNum;
   }
}
